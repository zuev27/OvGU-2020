...
while dP >= eps
  while Jak2 >= 0
    ...
    % Voltage in node, kV
    Table(i+1,6) = result.detJ(1,1);               % Jacobian |J|
    Table(i+1,7) = Table(i+1,6)/dP;                % First-order derivatives of Jacobian |J'|
    Table(i+1,8) = (Table(i,7)-Table(i+1,7))/dP;   % Second-order derivatives of Jacobian |J''|
...
Table(1,8) = 1.05*Table(2,8);   % First value of second-order derivatives in table 
                                % equal 1.05 of second value for correct calculations

Table(1,9) = 1.05*Table(2,9);   % First value of Taylor series expansion in table 
                                % equal 1.05 of second value for correct calculations                                                      
...
grid minor;
hold on;
plot(Table(1:i,3),Table(1:i,7),'-ob','LineWidth',2,'MarkerSize',4);  % Second-order derivatives of Jacobian |J|''(P)
title(['{\it J"(P), node }' num2str(N_load)]);
xlim([Table(1,3) inf]);
xlabel('{\it P}_{load} [MW]');
ylabel('{\it J"} [MW^{ -2}]');
