...
%% Automatic load adding of load nodes
...
while dP >= eps
  while Jak2 >= 0
    ...
    Table(i+1,5) = result.detJ(1,1);               % Jacobian |J|
    Table(i+1,6) = Table(i+1,5)/dP;                % First derivatives of Jacobian |J'|
    Table(i+1,7) = (Table(i,6)-Table(i+1,6))/dP;   % Second derivatives of Jacobian |J''|
    Table(i+1,8) = Table(i+1,5) + Table(i+1,6)*dP + Table(i+1,7)*dP^2; 
    % Taylor series expansion of Jacobian               
    mpc.bus(N_load, 3) = mpc.bus(N_load, 3) + dP;
  end;
            
if Table(i+1,4) < 0.7     % If the voltage value less than value equal 0.7,                                                                                              
   Table(i+1,4) = 0.7;    % it is accepted a value of 0.7
end;                      % for correct calculations
...
%% Making assumptions in the regim calculation
Table(1,:) = [ ];               % Delete the first row of the table
    
Table(1,7) = 1.1*Table(2,7);    % First value of second derivatives in table 
                                % equal 1.1 of second value for correct calculations

Table(1,8) = 1.05*Table(2,8);   % First value of Taylor series expansion in table 
                                % equal 1.05 of second value for correct calculations                                                      
%% Searching extreme mode
...
grid minor;
hold on;
plot(Table(1:i,3),Table(1:i,5),'-ob','LineWidth',2,'MarkerSize',4);  % Jacobian |J|(P)
plot(Table(1:i,3),Table(1:i,8),'--or','LineWidth',2,'MarkerSize',4); % Taylor series expansion of Jacobian
title(['{\it |J|(P), node }' num2str(N_load)]);
xlim([Table(1,3) inf]);
xlabel('{\it P}_{load} [MW]');
ylabel('{\it |J|} [-]');
legend('Jacobian','Taylor series of expansion of Jacobian');
...
grid minor;
hold on;
plot(Table(1:i,3),Table(1:i,7),'-ob','LineWidth',2,'MarkerSize',4);  % Second derivatives of Jacobian |J|''(P)
title(['{\it J"(P), node }' num2str(N_load)]);
xlim([Table(1,3) inf]);
xlabel('{\it P}_{load} [MW]');
ylabel('{\it J"} [MW^{ -2}]');
...
grid minor;
hold on;
plot(Table(1:i,3),Table(1:i,4),'-ob','LineWidth',2,'MarkerSize',4);  % Voltage in node U(P)
