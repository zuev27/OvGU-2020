 ...
 %% Automatic weighting of load nodes
        ...
        while dP >= eps && degrees <= 90
            while Jak2 >= 0
                ...
                Tabl(i+1,7) = result.bus(N_load, 9);             % Degrees
    ...
    disp(['Jacobian 1 = ' num2str(Jak1) '.']);
    disp(['Jacobian 2 = ' num2str(Jak2w) '.']);
...
hold on;
plot(Tabl(1:i,3),Tabl(1:i,4),'-ob'); % Jacobian J(P)
...
set(0,'DefaultTextFontSize',10,'DefaultTextFontName','Times New Roman'); 
hold on;
plot(Tabl(1:i,3),Tabl(1:i,6),'-ob'); % Second derivatives of Jacobian J''(P)
title(['{\it J"(P)}, node ' num2str(N_load)]);
xlabel('{\it P}_{load} [MW]');
ylabel('{\it J"} [-]');